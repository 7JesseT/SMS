...
import { useTeacherPrayerSchedule } from '../../services/teacherApi';
import { useAuth } from '../../context/AuthContext';
import { useNavigate } from 'react-router-dom';

const TeacherPrayerSchedulePage: React.FC = () => {
  ...
  // Get schoolId from currentUser
  const currentUser = getCurrentUserData();
  const schoolId = typeof currentUser?.school === 'string' 
    ? currentUser.school 
    : currentUser?.school?._id;

  const { data: prayers, isLoading, error } = useTeacherPrayerSchedule(schoolId);
      ...
      >
        <Container maxWidth="lg">
          ...
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            >
              <MosqueIcon sx={{ fontSize: 40, color: 'white' }} />
            </Box>
            ...
            {sortedPrayers.length === 0 ? (
              <Box sx={{ textAlign: 'center', py: 6 }}>
                ...
                <Typography variant="body2" color="text.secondary">
                  Prayer schedule has not been set up yet
                </Typography>
              </Box>
            ) : (
              <List sx={{ p: 0 }}>
...
export default TeacherPrayerSchedulePage;
